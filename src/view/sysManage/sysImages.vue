  ...
  <card style="height: 100%" >
    ...
    <Form ref="findForm" :model="searchData" :label-width="72" inline>
      ...
      <FormItem label="图片名称：" prop="name">
        <Input  v-model="searchData.name" placeholder="请输入图片名称..." clearable style="width: 190px" />
      </FormItem>
      <FormItem label="图片类型：" prop="type">
        <Input  v-model="searchData.type" placeholder="请输入图片类型..." clearable style="width: 190px" />
      </FormItem>
      <FormItem label="图片状态：" prop="status" >
    ...
    <Drawer title="addMode" v-model="insertMode" width="500" :mask-closable="false" :styles="styles">
      ...
      <Form ref="insertForm" :model="insertData"  :rules="submitRules" :label-width="130">
        <FormItem label="图片名称" prop="name">
          <Input v-model="insertData.name" placeholder="请输入图片名称..." style="width:310px" />
        </FormItem>
        <FormItem label="图片类型" prop="type">
          <Input v-model="insertData.type" placeholder="请输入图片类型..." style="width:310px" />
        </FormItem>
        <FormItem label="图片路径" prop="url" >
          <upload-pic-input :defaultImages="insertData.url" placeholder="请输入图片路径..." @on-change="handleUpload" width="200px" ref="upload"></upload-pic-input>
        </FormItem>
        <FormItem label="图片状态：" prop="status" >
    ...
    <Drawer title="updateMode" v-model="modifyMode" width="500" :mask-closable="false" :styles="styles">
      ...
      <Form ref="updateForm" :model="updateData"  :rules="submitRules" :label-width="130">
        ...
        <FormItem label="图片名称" prop="name">
          <Input v-model="updateData.name" placeholder="请输入图片名称..." style="width:310px" />
        </FormItem>
        <FormItem label="图片类型" prop="type" >
          <Input v-model="updateData.type" placeholder="请输入图片类型..." style="width:310px" />
        </FormItem>
        <FormItem label="图片路径" prop="url" >
          <upload-pic-input :defaultImages="updateData.url" placeholder="请输入图片路径..." @on-change="handleUpload" width="200px" ref="upload"></upload-pic-input>
        </FormItem>
...
export default {
  ...
  methods: {
    ...mapGetters('sysImages', []),
    ...mapMutations('sysImages', ['selection']),
    ...mapActions('sysImages', ['findDataAction', 'insertFormData', 'updateFormData', 'getModifyData', 'deleteFormData']),
    handleSubmit (name) {
    ...
    handleUpload (v) {
      this.insertData.url = v
      this.updateData.url = v
    }
  }
}
</script>
